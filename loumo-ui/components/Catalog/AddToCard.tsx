...
export function AddToCard({ children, product }: Props) {
    ...
    const addToCart = () => {
        console.log("Ajouter au panier :", { productId: product?.id, variant, quantity })
        addOrderItem({ variant: variant!, note: "" })
        ...
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="flex flex-row gap-3">
                    {variant?.imgUrl ? (
                        <img src={variant.imgUrl} alt={variant.name} className="w-[75px] h-auto aspect-square" />
                    ) : (
